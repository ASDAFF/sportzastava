...
$arResult['SHOW_PREVIEW_DESCRIPTION'] = false;
    ...
    switch ($arParams['DISPLAY_PREVIEW_TEXT_MODE']) {
        case 'E':
            if (!isset($arResult['DETAIL_TEXT']) || strlen($arResult['DETAIL_TEXT']) < 1)
                $arResult['SHOW_PREVIEW_DESCRIPTION'] = true;
            break;
        case 'S':
            $arResult['SHOW_PREVIEW_DESCRIPTION'] = true;
            break;
    ...
    && !empty($arParams['VIDEO_IBLOCK_PROPERTY']))
{
    ...
    while ($CIBElement = $videos->GetNextElement()) {
        ...
        $link = ArrayHelper::getValue($elementProperties, [$arParams['VIDEO_IBLOCK_PROPERTY'], 'VALUE']);
        if (!empty($link)) {
            $videoLinks[] = [
                'ID' => $CIBElement['ID'],
                'NAME' => $CIBElement['NAME'],
                'URL' => $link
            ];
        }
    }
