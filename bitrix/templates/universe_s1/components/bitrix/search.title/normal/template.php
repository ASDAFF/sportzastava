...
<?} else {?>
    <div class="intec-search-normal <?=($arParams['POSITION_SEARCH']=='header')?'intec-search-normal-header':'intec-search-normal-top'?>">
        <div id = "<?=$CONTAINER_ID?>" class="intec-search-normal-wrapper">
            <form action = "<?=$arResult["FORM_ACTION"]?>" class="intec-search-form">
                <button type="submit" class="intec-search-icon glyph-icon-loop intec-cl-text" aria-hidden="true"></button>
                <input id = "<?=$INPUT_ID?>" class="intec-search-input" type = "text" name = "q" value = "" maxlength="50"  autocomplete = "off" placeholder="<?=GetMessage('SEARCH_INPUT_PLACEHOLDER_NRM');?>"/>
            </form>
        </div>
    </div>
