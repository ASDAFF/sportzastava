...
$orderPropGroups = CSaleOrderPropsGroup::GetList(
    array(),
    array('*'),
    false,
    false,
...
while ($group = $orderPropGroups->Fetch()) {
    $arGroup[$group['ID']] = '['.$group['ID'].'] '.$group['NAME'];
}
if (!empty($arCurrentValues['ORDER_GROUP_PROPS'])) {
    $rsOrderProp = CSaleOrderProps::GetList(
        array(),
        array('PROPS_GROUP_ID' => $arCurrentValues['ORDER_GROUP_PROPS']),
        false,
        false,
        array()
    );
    while ($arTempProp = $rsOrderProp->Fetch()) {
        $arOrderProp[$arTempProp['ID']] = '['.$arTempProp['ID'].'] '.$arTempProp['NAME'];
    }
}

$arTemplateParameters = array(
    'ORDER_GROUP_PROPS' => array(
        'NAME' => GetMessage('ORDER_GROUP_PROPS'),
        'TYPE' => 'LIST',
        'VALUES' => $arGroup,
        'PARENT' =>'BASE',
        'REFRESH' => 'Y'
    ),
	'ORDER_PROP_DELIVERY' => array(
	    'NAME' => GetMessage('ORDER_PROP_DELIVERY'),
        'TYPE' => 'LIST',
        'VALUES' => $arOrderProp,
        'PARENT' => 'BASE'
    ),
