...
class FileHelperBase
{
     ...
     * @return string
     */
    public static function getRelativePath($path, $fromPath, $ds = DIRECTORY_SEPARATOR)
    {
        $fromPath = static::normalizePath($fromPath, $ds);
        $fromPath = explode($ds, $fromPath);
        $pathCurrent = FileHelper::normalizePath($path, $ds);
        $pathCurrent = explode($ds, $pathCurrent);
        $offset = 0;

        foreach ($pathCurrent as $key => $partCurrent) {
            $part = ArrayHelper::getValue($fromPath, $key);
        ...
        if ($offset > 0) {
            $pathCurrent = ArrayHelper::slice($pathCurrent, $offset);
            $length = count($fromPath) - $offset;

            for ($i = 0; $i < $length; $i++)
                ArrayHelper::unshift($pathCurrent, '..');
        }

        $pathCurrent = implode($ds, $pathCurrent);

        if ($offset > 0)
            $pathCurrent = $ds.$pathCurrent;
