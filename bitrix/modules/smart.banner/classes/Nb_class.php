...
class Nb_class
{
    function GetList($arSort = Array("SORT"=>"ASC"),$arFilter=Array(),$arSelect = Array())
    {
        ...
        $sql = "SELECT $selectFields FROM b_smart_banner";
        if(strlen($where)>0)
            $sql .= " WHERE $where";
        if(strlen($order)>0)
            $sql .= " ORDER BY $order";
    ...
    function copyRow($ID){
        ...
        if($ID>0)
        {
            ...
            while($res = $resData->NavNext(true, "f_")){
                return $ID = $DB->Add("b_smart_banner",$res);
            }
        }
    }
    ...
    function Add(&$arFields)
    {
        ...
        return $ID = $DB->Add("b_smart_banner",$arFields);
    ...
    function Update($ID,$arFields)
    {
        ...
        $strUpdate = $DB->PrepareUpdate("b_smart_banner", $arFields);
        if($strUpdate!="")
        {
            $strSql = "UPDATE b_smart_banner SET ".$strUpdate." WHERE ID=".$ID;
    ...
    function SaveFile($arImage,$ID)
    {
        if (is_array($arImage))
        {
            $arImage["MODULE_ID"] = "smart.banner";

            if ($ID>0)
            {
                global $DB;

                $rsBanner= $DB->Query("SELECT IMAGE_ID FROM b_smart_banner WHERE ID='$ID'", false, "<b>Error in </b><br/>File: ".__FILE__."<br/>Line: ".__LINE__."<br/>");
    ...
    function ShowBanner(){
        ...
        global $APPLICATION;
        $sql = "SELECT * FROM b_smart_banner";
        global $DB;
        $resData = $DB->Query($sql,false);
        while($res = $resData->NavNext(true, "f_")){
    ...
    function Delete($ID)
    {
        ...
        if($ID>0)
        {
            $sql = "SELECT ID,IMAGE_ID FROM b_smart_banner WHERE ID = '$ID'";
            ...
            if ($Banner = $Banner->Fetch())
            {
                ...
                $strSql = "DELETE FROM b_smart_banner WHERE ID = '{$Banner["ID"]}'";
                $DB->Query($strSql, false, "File: ".__FILE__."<br>Line: ".__LINE__);
                return true;
            }
        }
    ...
    function statUpdate($ID,$strUpdate){
        ...
        {
            $strSql = "UPDATE b_smart_banner SET ".$strUpdate." = ".$strUpdate." + 1 WHERE ID = '$ID'";
            $DB->Query($strSql, false, "File: ".__FILE__."<br>Line: ".__LINE__);
            return true;
        }
    }
